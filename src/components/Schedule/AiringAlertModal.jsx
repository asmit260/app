...
export default function AiringAlertModal({
  ...
  useEffect(() => {
    if (existingAlert?.lead_minutes !== undefined) {
      setLeadMinutes(existingAlert.lead_minutes);
    }
  }, [existingAlert]);
  ...
  const modalContent = (
    ...
    >
      <div 
        className="card-manga-panel bg-sand-50 dark:bg-sand-100 max-w-sm w-full p-5 sm:p-6 relative rounded-lg border-2 border-stone-900 shadow-manga-lg overflow-hidden space-y-4 max-h-[90vh] overflow-y-auto hide-scrollbar"
      ...
      >
        ...
        <div className="flex gap-3 items-center pr-6">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1 text-xs font-mono text-stone-600 dark:text-stone-300 font-bold">
              <Clock className="w-3.5 h-3.5 text-navy-700 dark:text-amber-400" />
          ...
          <div className="grid grid-cols-2 gap-2">
            {LEAD_OPTIONS.map((opt) => (
              <button
                ...
                className={`py-2 px-2 rounded-md text-xs font-bold border-2 border-stone-900 transition-all text-center ${
                  leadMinutes === opt.value
                    ? 'bg-amber-400 text-ink-900 shadow-[2px_2px_0px_0px_rgba(24,19,13,1)] scale-[1.02]'
                    : 'bg-sand-100 dark:bg-sand-200 text-stone-700 hover:bg-sand-200'
                }`}
              >
                {opt.label}
              </button>
